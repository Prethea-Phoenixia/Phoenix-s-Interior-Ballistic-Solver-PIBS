...
conditions is {v_j:.4g} m/s."
        ...
        def _f_p_e_1(e_1: float) -> tuple[float, float, float, float, float]:
            ...
            def _f_p_z(z: float) -> tuple[float, float, float, float, float]:
                ...
                r = []
                t_bar, l_bar, v_bar = rkf(d_func=_ode_z, ini_val=ys, x_0=x, x_1=z, rel_tol=self.tol, record=r)[1]
        ...
        e_1_solved, _ = dekker(
            lambda _e_1: _f_p_e_1(_e_1)[0], probe_web, next_probe_web, y_rel_tol=self.tol  # >0  # ?0
        )
