        ...
        **_,
        ...
        def _f_p_bar(z: float, l_bar: float, v_bar: float, eta: float, tau: float) -> float:
            ...
            if self.control == POINT_PEAK_AVG:
                ...
                h = min(math.inf if v_bar == 0 else (vb / (v_j * v_bar)), 2 * self.phi_1 * self.m / (w * (1 - eta)) + 1)
            ...
            if peak is None:
                ...
                def _f_p_z(z: float) -> tuple[float, float, float, float, float, float, float]:
                    ...
                    t_bar, l_bar, v_bar, eta, tau = rkf(
                        d_func=_ode_z, ini_val=ys, x_0=x, x_1=z, rel_tol=self.tol, record=r
                    )[1]
                ...
                z_p = 0.5 * sum(gss(lambda z: _f_p_z(z)[0], z_j, z_k, x_tol=self.tol, find_min=False))

                return _f_p_z(z_p)

        probe_web = 0.5 * self.min_web

        p_bar_max = (self.f * delta / (1 - self.alpha * delta)) / (self.f * delta)
        if p_bar_max <= p_bar_d:
            raise ValueError("Design pressure cannot be achieved by varying web down to minimum")

        dp_bar_probe = next_dp_bar_probe = _f_p_e_1(probe_web)[0]
        ...
        while dp_bar_probe * next_dp_bar_probe > 0:  # is same sign
            ...
            next_dp_bar_probe = _f_p_e_1(next_probe_web)[0]

        e_1_solved, _ = dekker(lambda _e_1: _f_p_e_1(_e_1)[0], probe_web, next_probe_web, y_rel_tol=self.tol)
